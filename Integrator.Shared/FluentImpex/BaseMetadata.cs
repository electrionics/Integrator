...
namespace Integrator.Shared.FluentImpex
{
        ...
        where TImport : new()
    {
        ...
        private List<string> CellOrders { get; set; }
        ...
        public List<string?> CreateStrings(TImport entity)
        {
            var result = new List<string?>();

            foreach (var columnName in CellOrders) 
            {
                if (CellValues.TryGetValue(columnName, out var prop))
                {
                    var nonConvertedValue = prop.GetValue(entity);
                    
                    result.Add(ConvertValue(nonConvertedValue?.GetType() ?? typeof(object), nonConvertedValue));
                }
            }
        ...
        {
            return CellOrders.ToList();
        }

        private string? ConvertValue(Type propertyType, object? value)
        {
            if (Converters.TryGetValue(propertyType, out var converter))
            {
